...
namespace KGM_Misafirhane
{
    public partial class KayitliMusteriler : Form
    {
        ...
        public static string adSoyad = "", Email = "", Telefon = "", MusteriTipi = "";
        ...
        private void KayitliMusteriler_Load(object sender, EventArgs e)
        {
            ...
            bg.Open();
            SqlCommand cmd = new SqlCommand("select distinct CustomerName from Customers", bg);
        ...
        private void lstViewOdalar_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string musteriTipi = "";
            bg.Open();
            SqlCommand cmd = new SqlCommand("select DISTINCT CustomerName, * from Customers where CustomerName='" + adSoyad + "'", bg);
            SqlDataReader oku = cmd.ExecuteReader();
            while (oku.Read())
            {
            ...
            foreach (string columnName in columnNames)
            {
                ...
                bg.Open();
                cmd = new SqlCommand("select * from Reservations where CustomerId in(select ReservationId from Customers where CustomerName='" + adSoyad + "')", bg);
                oku = cmd.ExecuteReader();
                while (oku.Read())
                {
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            SqlConnection bg = new SqlConnection(ConnectionString);
            bg.Open();
            SqlCommand komut = new SqlCommand("UPDATE Customers SET CustomerType='" + cmbBxMusteriTipi.Text + "', CustomerName='" + txtBxAdSoyad.Text + "', CustomerMail='" + txtBxEmail.Text + "', CustomerPhone='" + txtBxTelefon.Text + "' WHERE CustomerName='" + adSoyad + "'", bg);
