...
namespace KGM_Misafirhane
{
    public partial class Odalar : Form
    {
        ...
        public static int kat = 0, odaNo = 0;
        public static bool odaSecildiMi = false;

        public static string adSoyad = "", email = "", telefon = "", yetiskinSayisi = "", cocukSayisi = "", odaTuru = "";
        public static int katG = 0, odaNoG = 0, odaSayisi = 0, fiyat = 0;
        public static DateTime GirisTarih = DateTime.Now, CikisTarih = DateTime.Now;
        public static bool iptalEt = false;

        public static string odaDoluMu = "", iptalEmail = "";
        ...
        private void btnSureyiUzalt_Click(object sender, EventArgs e)
        {
            ...
            if (odaSecildiMi == true)
            {
                if (cmbxSüreyiUzat.Text != "")
                {
                    ...
                    for (int i = 0; i<6;i++)
                    {
                        if (süreyiUzalt == i + " Gün")
                        {
                            bg.Open();
                            SqlCommand komut = new SqlCommand("UPDATE Reservations SET CheckOut = DATEADD(DAY, " + i + ", CheckOut) WHERE Status='Oturmakta' and RoomFloor = " + kat + " and RoomNo = " + odaNo + "", bg);
                            komut.ExecuteNonQuery();
                            bg.Close();
                        }
                        if (süreyiUzalt == i + " Hafta")
                        {
                            bg.Open();
                            SqlCommand komut = new SqlCommand("UPDATE Reservations SET CheckOut = DATEADD(WEEK, " + i +", CheckOut) WHERE Status='Oturmakta' and RoomFloor = " + kat + " and RoomNo = " + odaNo + "", bg);
                            komut.ExecuteNonQuery();
                            bg.Close();
                        }
                        if (süreyiUzalt == i + " Ay")
                        {
                            bg.Open();
                            SqlCommand komut = new SqlCommand("UPDATE Reservations SET CheckOut = DATEADD(MONTH, " + i + ", CheckOut) WHERE Status='Oturmakta' and RoomFloor = " + kat + " and RoomNo = " + odaNo + "", bg);
                            komut.ExecuteNonQuery();
                            bg.Close();
                        }
                    }
        ...
        private void btnSureyiAzalt_Click(object sender, EventArgs e)
        {
            ...
            if (odaSecildiMi == true)
            {
                ...
                {
                    string süreyiAzalt = cmbxSüreyiAzalt.Text;

                    for (int i = 0; i < 6; i++)
                    {
                        if (süreyiAzalt == i + " Gün")
                        {
                            bg.Open();
                            SqlCommand komut = new SqlCommand("UPDATE Reservations SET CheckOut = DATEADD(DAY, -" + i + ", CheckOut) WHERE Status='Oturmakta' and RoomFloor = " + kat + " and RoomNo = " + odaNo + "", bg);
                            komut.ExecuteNonQuery();
        ...
        private void btnOde_Click(object sender, EventArgs e)
        {
            ...
            while (oku.Read())
            {
                girisTarihi = (DateTime)oku["CheckIn"];
                cikisTarihi = (DateTime)oku["CheckOut"];
                ucret = (int)oku["Price"];
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    if (email != "")
                    {
                        try
                        {
                            ...
                            string body = "Sayın " + adSoyad + ", " + girisTarihi.ToString("dd/MM/yyyy") + " - " + cikisTarihi.ToString("dd/MM/yyyy") + " Tarihleri Arasında Sizleri Misafirhanemizde Misafir Etmekten Mutluluk Duyduk Tekrar Görüşmek Üzere İyi Günler:)";
